...
public class ReminderHelper extends BaseHelper {
    ...
    private void swipeUntilNeededYear(String year, double starPoint, double stopPoint) {
        while (!getYear().equals(year)) {
            swipeByElement(By.className("android.widget.ListView"), starPoint, stopPoint);
            getYear();
        }
    }
    ...
    public void selectTime(String timeOfDay, int xHour, int yHour, int xMin, int yMin) {
        ...
        if (timeOfDay.equals("am")) {
            tapeWithCoordinates(285, 1324);
        } else if (timeOfDay.equals("pm")) {
            tapeWithCoordinates(801, 1324);
        }
        tapeWithCoordinates(xHour, yHour);
        tapeWithCoordinates(xMin, yMin);
    }
